...
<p align="center">
  <b>17 agents</b> · <b>27 tree-sitter languages</b> · <b>100+ via regex fallback</b> · <b>74 MCP tools</b> · <b>91s Linux kernel</b>
</p>
...
| `codeloom --decide "title" --reason "..."` | Record a decision (accepted/rejected) |
| `codeloom --reject "title" --reason "..."` | Record a rejected decision |
| `codeloom --hypothesis "title"` | Record an open hypothesis |
| `codeloom --lesson "text"` | Record a lesson/trap: tried and failed because |
| `codeloom --supersede OLD NEW` | Mark a decision as superseded |
| `codeloom --query-memory "topic"` | Search long-term memory: what do we already know about X |
| `codeloom --mark-seen f sym` | Mark files/symbols as deeply understood (hot set) |
| `codeloom --working-state` | Layered working-state packet: goal, decisions, actions, open items, hot set |
| `codeloom --cognitive-load "topic"` | Cognitive-load-aware decomposition (intrinsic/extraneous/germane) |
...
codeloom --context-card A B   # batch triage card for MULTIPLE symbols in ONE call
codeloom --answer "question"  # one-call cited answer with honest confidence
codeloom --why "topic"        # decision lookup stamped [exact]/[fuzzy]/[unverified]
...
Exposes **74 tools**: `codeloom_map`, `codeloom_graph`, `codeloom_focus`,
...
`codeloom_trace`, `codeloom_ask` (single natural-language entry point that
routes deterministically across the **entire tool surface** — task, symbol,
call-graph, routes, channels, framework, rename, refactor, similar, deadcode,
precision, memory, checkpoint, adr, resume, seen, session, export, cross-repo,
context-diff, files, langs — so the agent's effective surface is **1 tool**,
not 61),
