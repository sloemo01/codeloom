...
- Latest release: `https://github.com/sloemo01/codeloom/releases/tag/v0.58.0`
...
**6/6** Repo: https://github.com/sloemo01/codeloom — MIT, one file, CI-verified on Linux/macOS/Windows, v0.58.0 released. `--resume` restores your agent's structural context after a compaction. Code-embedded task brief (`--ask`/`--pack`), edit-relevance, 99% token savings, lazy per-symbol index + `--watch`/`--watch-core` (near-resident or native, no daemon), 62 MCP tools, integrated C engine + `--serve` — indexes the full Linux kernel (3.2M symbols, 408k edges) in **~91s**. Go use it.
