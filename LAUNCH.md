...
- Latest release: `https://github.com/sloemo01/codemap/releases/tag/v0.13.0`

---

## Show HN post

**Title:** Show HN: codemap — task-aware codebase intelligence for AI agents, in one stdlib file
...
I kept hitting this, so I built codemap: one file, zero dependencies, no daemon, 100% local. It gives an agent a real structural model of a repo in under a second — and, unlike the search tools, it tells the agent *what's relevant to the task* and *what breaks if it changes something*.
...
- **MCP server** — `codemap-mcp.py` is a zero-dep MCP server with 19 tools, so agents call it natively
...
Would love feedback — especially on the task-relevance ranking and what else agents actually need to stop burning tokens.
...
**3/6** The part the search tools don't do — task-orientation:
...
**4/6** Retrieval too, token-efficient:
• `--read X` → exact symbol source (multi-language, no whole-file reads)
• `--search` / `--usages` / `--grep` → find any symbol, usage, or snippet
• `--explain` / `--similar` / `--deadcode` → understand & refactor

**5/6** The MCP server is zero-dependency too — pure stdlib JSON-RPC over stdio, 19 tools. Register it in Claude Code / Cursor / Codex and your agent calls codemap natively. No `pip install`, no index, always fresh.

**6/6** Repo: https://github.com/sloemo01/codemap — MIT, one file, CI-verified on Linux/macOS/Windows, v0.13.0 released. Go use it. Feedback welcome, especially on the task-relevance ranking.
